...
sshkeydn="$(ldapsearch -x -H $url -b "$base" -D "$binddn" -w $passwd '(&(objectClass=posixAccount)(uid='"$1"'))' 'sshkey' | sed -n '/^ /{H;x};/sshkey:/x;$g;s/\n *//g;s/sshkey: //gp')"


for a in $sshkeydn
do
    ldapsearch -x -H $url -b "$a" -D "$binddn" -w $passwd '(objectClass=sshPublicKey)' 'sshpubkey' | sed -n '/^ /{H;x};/sshpubkey:/x;$g;s/\n *//g;s/sshpubkey: //gp'
done
