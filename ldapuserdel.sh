...
groupsdn=$(ldapsearch -x $ldapurl -D "$binddn" -w "$bindpasswd" -b "$basedn" "(&(objectClass=account)(uid=$username))" -LLL | grep -P "^memberOf:" | awk '{print $2}')
...
gidgroupdn=$(ldapsearch -x $ldapurl -D "$binddn" -w "$bindpasswd" -b "$basedn" "(&(objectClass=groupOfNames)(gidNumber=$gid))" -LLL | grep -P "^dn:" | awk '{print $2}')
...
if [ "$(ldapsearch -x $ldapurl -D "$binddn" -w "$bindpasswd" -b "$basedn" "(&(objectClass=groupOfNames)(gidNumber=$gid))" -LLL | grep -P "^memberUid:")" = "" ]
then
	ldapdelete -x $ldapurl -D "$binddn" -w "$bindpasswd" $gidgroupdn
else
	echo "$0: group $(ldapsearch -x $ldapurl -D "$binddn" -w "$bindpasswd" -b "$basedn" "(&(objectClass=groupOfNames)(gidNumber=$gid))" -LLL | grep -P "^cn:" | awk '{print $2}') not removed because it has other members."
fi
